import { Link } from 'react-router-dom'
import { FiPhone, FiMail, FiMapPin } from 'react-icons/fi'
import { FaWhatsapp, FaInstagram, FaFacebookF, FaYoutube } from 'react-icons/fa'
import logo from '../../assets/logo.jpg'
import siteConfig from '../../config/siteConfig'
import { whatsappLink, telLink } from '../../utils/whatsapp'
import './Footer.css'

const quickLinks = [
  { to: '/', label: 'Home' },
  { to: '/about', label: 'About Us' },
  { to: '/onion-seeds', label: 'Onion Seeds' },
  { to: '/gallery', label: 'Gallery' },
  { to: '/contact', label: 'Contact Us' }
]

export default function Footer() {
  const { contact, social, company } = siteConfig
  const isSet = (url) => url && !url.endsWith('_URL')

  const socials = [
    isSet(social.instagram) && { href: social.instagram, label: 'Instagram', Icon: FaInstagram },
    isSet(social.facebook) && { href: social.facebook, label: 'Facebook', Icon: FaFacebookF },
    isSet(social.youtube) && { href: social.youtube, label: 'YouTube', Icon: FaYoutube },
    { href: whatsappLink(), label: 'WhatsApp', Icon: FaWhatsapp }
  ].filter(Boolean)

  return (
    <footer className="footer">
      <div className="shell footer__grid">
        <div className="footer__brand">
          <div className="footer__id">
            <span className="logo-chip footer__logo">
              <img src={logo} alt="GATEE SEEDS Pvt. Ltd. logo" width="70" height="70" loading="lazy" />
            </span>
            <span>
              <strong>GATEE SEEDS</strong>
              <small>PVT. LTD.</small>
            </span>
          </div>
          <p>{company.description}</p>

          <ul className="footer__social">
            {socials.map(({ href, label, Icon }) => (
              <li key={label}>
                <a href={href} aria-label={label} target="_blank" rel="noopener noreferrer">
                  <Icon aria-hidden="true" />
                </a>
              </li>
            ))}
          </ul>
        </div>

        <nav className="footer__col" aria-label="Footer">
          <h2>Quick links</h2>
          <ul>
            {quickLinks.map((link) => (
              <li key={link.label}>
                <Link to={link.to}>{link.label}</Link>
              </li>
            ))}
            <li>
              <a href="/#why-choose-us">Why Choose Us</a>
            </li>
          </ul>
        </nav>

        <div className="footer__col">
          <h2>Contact</h2>
          <ul className="footer__contact">
            <li>
              <FiPhone aria-hidden="true" />
              <span>
                <a href={telLink(contact.phonePrimary)}>{contact.phonePrimaryDisplay}</a>
                <a href={telLink(contact.phoneSecondary)}>{contact.phoneSecondaryDisplay}</a>
              </span>
            </li>
            <li>
              <FaWhatsapp aria-hidden="true" />
              <a href={whatsappLink()} target="_blank" rel="noopener noreferrer">
                WhatsApp enquiry
              </a>
            </li>
            <li>
              <FiMail aria-hidden="true" />
              <a href={`mailto:${contact.email}`}>{contact.email}</a>
            </li>
            <li>
              <FiMapPin aria-hidden="true" />
              <address>
                {contact.addressLines.slice(1).map((line) => (
                  <span key={line}>{line}</span>
                ))}
              </address>
            </li>
          </ul>
        </div>
      </div>

      <div className="footer__bar">
        <div className="shell footer__barInner">
          <p>© {new Date().getFullYear()} GATEE SEEDS PVT. LTD. All rights reserved.</p>
          <p>Onion seed producers and marketers.</p>
        </div>
      </div>
    </footer>
  )
}
